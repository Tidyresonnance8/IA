    ...
    while not Game.is_terminal(s):
        ...
        if earlyStopping == 1 and depth >= earlyStop: # early stopping heuristique 
            ...
            if(s.current_player == 0):
                return (score, 1-score), i
            else:
                return (1-score, score), i
...
class AgentMCTSFinal(Agent):
    def __init__(self, player, constanteUCT:int = 1.41, earlyStopping:int = 1, earlyStop:int = 18, intelligentsPlayouts:int = 0, intelligentTimeUse:bool = True, maxTime:int = 300):
    ...
    def act(self, state, remaining_time):
        ...
        while time.perf_counter() < deadline:
            ...
            utility, i = simulate(node.state, self.intelligentsPlayouts, self.earlyStopping,  self.earlyStop)
            self.iterCount += i
